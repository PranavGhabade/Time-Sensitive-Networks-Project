...
void send_stream(const std::string& iface, const std::string& src_mac,
    ...
    *(uint16_t*)&ether_ptr[12] = htons(ETHER_TYPE_VLAN);     // VLAN tag

    // VLAN Header
    uint16_t vlan_tag = (0 << 13) | (stream_cfg.dscp << 5) | (stream_cfg.vlan_id & 0xFFF);
    *(uint16_t*)&ether_ptr[14] = htons(vlan_tag);
    *(uint16_t*)&ether_ptr[16] = htons(ETHER_TYPE_IPV4);     // Next EtherType = IPv4
    ...
    while (true) {
        ...
        payload << "[" << stream_cfg.name << "] #" << counter << " @ " << timestamp;
        std::string data = payload.str();

        iphdr *ip = (iphdr*)(frame + 18);
        udphdr *udp = (udphdr*)(frame + 18 + sizeof(iphdr));
        char *msg = (char*)(frame + payload_offset);
        ...
        ip->tot_len = htons(sizeof(iphdr) + sizeof(udphdr) + data.size());
        ip->id = htons(0);
        ip->frag_off = 0;
        ip->ttl = 64;
        ...
        ip->check = checksum((uint16_t*)ip, sizeof(iphdr)/2);
        ...
        packet_len = payload_offset + data.size();

        if (sendto(sockfd, frame, packet_len, 0,
                   (struct sockaddr*)&socket_address, sizeof(sockaddr_ll)) < 0) {
            perror("sendto");
        } else {
            std::cout << "[" << stream_cfg.name << "] sent to port "
                      << stream_cfg.port << ": " << data << std::endl;
        }
...
int main() {
    ...
    for (auto& [name, val] : config["traffic_streams"].items()) {
        StreamConfig s;
        s.name = name;
        s.interval_ms = val["interval_ms"];
        s.vlan_id = val["vlan_id"];
        s.dscp = val["dscp"];
        s.port = val["port"];
